...
\def\mybox@@#1#2#3#4{\unskip\ {\Large \fboxrule=0.1mm\fboxsep=0mm%
  \ifnum#1>0\fcolorbox{blue}{b_blue}{\vrule width 0pt height .88\zw depth .12\zw\kern#1\dimexpr.25\zw}\kern-\fboxrule\fi%
  \fcolorbox{blue}{white}{\hbox{\<\ltjjachar#4\<}}%
  \ifnum#2>0\kern-\fboxrule\fcolorbox{blue}{b_blue}{\vrule width 0pt height .88\zw depth .12\zw\kern#2\dimexpr.25\zw}\fi%
}\ }
...
\def\RULER#1#2{%
  ...
  \rlap{\smash{\phantom{#1}\ \color{cyan}%
    ...
    \loop\ifnum\@tempcnta<20
       \kern-.05mm\vrule height \dimexpr-\baselineskip+0.88\zw depth \dimexpr #2\baselineskip+0.12\zw width0.1mm\kern-.05mm%
...
\item[\Param{rubypreintrusion}\ttfamily =\{<chr\_code>, <pre\_int>\}]
  ...
  \end{itemize}
  後者の状況の例を以下に載せる．
\begin{LTXexample}[width=0.333\textwidth, preset=\Large]
...
文字を区切る\emph{必要はない}（\verb+\kenten+内では「\texttt{|}」は特別な意味を持たない）
ことに注意してほしい．
...
進入は可能である．この\verb+\parindent+ 分のインデントへの\ActIntr{進入許容量}は
...
\begin{enumerate}[label=\emph{Step~\arabic*},labelindent=0pt,itemindent=*,itemsep=\medskipamount]
  ...
  \begin{enumerate}
    ...
    \medskip
    \emph{TODO: \Param{rubypreintrusion}, \Param{rubypostintrusion}が負のとき}
    \medskip
...
\[
 ...
 \Node{vlist $n_3$}{頭2}\longrightarrow
 \Node{vlist $n_4$}{頭1}\longrightarrow
 \Node{vlist $n_5$}{中}
\]
に置き換えておく．
...
\noalign{\hrulefill 行の境目\hrulefill}
\Node{vlist $n_4$}{頭1}\longrightarrow (\text{other nodes})\tag{行B}
\end{gather*}
\end{enumerate}
\end{enumerate}
...
周囲が\highLight{\ruby{あ|い|う|え}{たぶん|きっと|おそらく|maybe}}ないと思う
\end{LTXexample}
