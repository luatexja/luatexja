...
\item[\Param{rubypreintrusion}\ttfamily =\{<chr\_code>, <pre\_int>\}]
  ...
  \textbf{v0.5以降での変更点}：もし$-a$~($a>0$)を指定した場合は，次のようになる．
  \begin{itemize}
    \item 実際の\underline{前文字進入許容量}は$a$となる．
    \item 文字<chr\_code>の前のJFMグルー$G$に進入が発生し，
    かつ\underline{前文字進入許容量}が0でなかった場合は，JFMグルー$G$にはこのルビの進入は発生しない．
  \end{itemize}
  後者の状況がわかりにくいので
...
\item[\Param{rubypostintrusion}\ttfamily =\{<chr\_code>, <post\_int>\}]
  ...
  \textbf{v0.5以降での変更点}：もし$-a$~($a>0$)を指定した場合は，次のようになる．
  \begin{itemize}
    \item 実際の\underline{後文字進入許容量}は$a$となる．
    \item 実際の\underline{後文字進入量}が0でなかった場合は，文字<chr\_code>の後のJFMグルーにはルビの進入は発生しない．
  \end{itemize}
\end{description}
...
進入は可能である．この\verb+\parindent+ 分のインデントへの\underline{進入許容量}は
